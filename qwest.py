...
def scrape_team_results(team):
    url = "https://footystats.org/england/premier-league".format(team.lower().replace(" ", "-"))
    res = requests.get(url)
    soup = BeautifulSoup(res.text, "html.parser")
    results = []
    for result in soup.find_all("div", class_="matches__matches-list-container"):
        if "Result" in result.text:
            if team.lower() in result.text.lower():
                ...
                if "win" in result_text:
                    ...
                    results.append("L")
    return results[-5:] # return only the past 5 games
